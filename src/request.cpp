...
Request::Request(const QString url, QObject* parent)
    : QObject{ parent },
    manager(std::make_unique<QNetworkAccessManager>(this)),
    operations(std::make_unique<Operations>()),
    response(std::make_unique<Response>()),
...
void Request::addCourse(QObject * const course)
{
    try {
        const auto variables = this->operations->buildVariables(this->operations->addCourseKeys,course);
        qDebug() << variables;
    } catch (const std::exception& e) {
        qDebug() << e.what();
    }
